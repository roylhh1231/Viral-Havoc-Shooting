...
public class DoorWithKeys : MonoBehaviour, IInteractable
{
    ...
    [SerializeField] private GameObject Key; // Reference to the AccessCard GameObject
    ...
    {
        if (PlayerController.instance.allGuns[PlayerController.instance.currentGun].gameObject == Key)
        {
            // Remove the AccessCard from the allGuns list
            PlayerController.instance.RemoveGun(Key);
...
            }
        ...
        else
        {
            ...
            if (promptUI != null)
            {
                promptUI.Setup("Key needed");
                yield return new WaitForSeconds(3); // Wait for 3 seconds
                promptUI.Setup(_prompt); // Revert to initial prompt
            }
            Debug.Log("Access Denied: You need the Key to open this door.");
        }
    }
}
